...
int raw_write_run(struct fun_context *fctx)
{
    ...
    off_t dest_offset = strtoull(fctx->argv[1], NULL, 0) * BLOCK_SIZE;
    off_t len_written = 0;

    struct block_writer writer;
    OK_OR_CLEANUP(block_writer_init(&writer, fctx->output_fd, 120 * 1024, BLOCK_SIZE_LOG2));
...
int raw_memset_validate(struct fun_context *fctx)
{
    ...
    CHECK_ARG_UINT64_MAX(fctx->argv[2], INT32_MAX / BLOCK_SIZE, "raw_memset requires a non-negative integer block count");
...
int raw_memset_compute_progress(struct fun_context *fctx)
{
    ...
    // Count each byte as a progress unit
    fctx->progress->total_units += count * BLOCK_SIZE;
...
int raw_memset_run(struct fun_context *fctx)
{
    ...
    off_t dest_offset = strtoull(fctx->argv[1], NULL, 0) * BLOCK_SIZE;
    int count = strtol(fctx->argv[2], NULL, 0) * BLOCK_SIZE;
    int value = strtol(fctx->argv[3], NULL, 0);
    char buffer[block_size];
    memset(buffer, value, sizeof(buffer));

    struct block_writer writer;
    OK_OR_RETURN(block_writer_init(&writer, fctx->output_fd, 120 * 1024, BLOCK_SIZE_LOG2));
...
int mbr_write_run(struct fun_context *fctx)
{
    ...
    ssize_t written = aligned_pwrite(fctx->output_fd, buffer, BLOCK_SIZE, 0);
...
int uboot_clearenv_run(struct fun_context *fctx)
{
    ...
    ssize_t written = aligned_pwrite(fctx->output_fd, buffer, env.env_size, env.block_offset * BLOCK_SIZE);
...
int uboot_setenv_run(struct fun_context *fctx)
{
    ...
    ssize_t read = aligned_pread(fctx->output_fd, buffer, env.env_size, env.block_offset * BLOCK_SIZE);
