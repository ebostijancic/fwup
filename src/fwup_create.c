    ...
    if (assertions) {
        if (assertions->assert_gte >= 0 &&
            ...
            ERR_CLEANUP_MSG("file size assertion failed on '%s'. Size is %d bytes. It must be >= %d bytes (%d blocks)",
                            local_paths, total_len, assertions->assert_gte, assertions->assert_gte / BLOCK_SIZE);
        if (assertions->assert_lte >= 0 &&
                !(total_len <= assertions->assert_lte))
            ERR_CLEANUP_MSG("file size assertion failed on '%s'. Size is %d bytes. It must be <= %d bytes (%d blocks)",
                            local_paths, total_len, assertions->assert_lte, assertions->assert_lte / BLOCK_SIZE);
    }
...
static int add_file_resources(cfg_t *cfg, struct archive *a)
{
    ...
    while ((sec = cfg_getnsec(cfg, "file-resource", i++)) != NULL) {
        ...
        if (hostpath) {
            ...
            assertions.assert_lte = cfg_getint(sec, "assert-size-lte") * BLOCK_SIZE;
            assertions.assert_gte = cfg_getint(sec, "assert-size-gte") * BLOCK_SIZE;
